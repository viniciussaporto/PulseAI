...
async function createGuildCollection (guildId) {
  ...
  try {
    ...
    if (!collectionExists) {
      const initialData = { guildId, availableroles: [], removedroles: [] }
...
async function getRolesData (guildId) {
  ...
  try {
    ...
    if (document) {
      ...
      return { guildId, availableroles: [], removedroles: [] }
    }
  } catch (error) {
    console.error('Error connecting to MongoDB Atlas:', error)
    errorLogStream.write(`Error connecting to MongoDB Atlas: ${error}\n`)
    ...
    if (member.permissions.has('ADMINISTRATOR')) {
      ...
      if (subcommand === 'add') {
        ...
        if (!availableroles.includes(role.id)) {
          ...
          await updateRolesData(guild.id, availableroles, removedroles) // Fix the variable names here
        ...
        if (availableroles.includes(role.id)) {
          ...
          await updateRolesData(guild.id, updatedavailableroles, removedroles) // Fix the variable names here
...
async function updateRolesData (guildId, availableRoles, removedRoles) {
  ...
  try {
    ...
    // Update the document in the database with the new availableRoles and removedRoles arrays
    await collection.updateOne(
      { guildId },
      { $set: { availableRoles, removedRoles } },
      { upsert: true }
    )

    console.log(`Roles data updated for guild ${guildId}`)
  } catch (error) {
    console.error('Error connecting to MongoDB Atlas:', error)
  } finally {
    await client.close()
